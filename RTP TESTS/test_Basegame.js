...
// Nicht identisch zu WebCrypto, aber für Monte-Carlo-RTP sehr gut und schnell.
// ============================================================================
...
function runBaseGameSpin(rng) {
    ...
    const baseResult = runSpin(rng, false);
    ...
    if (baseResult.scatters >= 4) {
        ...
        while (fsLeft > 0) {
            ...
            const fsResult = runSpin(rng, true);
...
function runWorker() {
    ...
    for (let i = 0; i < spins; i++) {
        ...
        const result = runBaseGameSpin(rng);
        totalWin += result.win;
...
if (!isMainThread) {
    ...
    console.log("\n🚀 STARTE BASEGAME RTP SIMULATION 🚀");
    ...
    console.log("Math:                   Natural Cluster / keine künstlichen Cluster\n");
    ...
    for (let i = 0; i < numWorkers; i++) {
        ...
        worker.on("message", (msg) => {
            ...
            if (msg.type === "done") {
                const oldDone = workerProgress.get(msg.workerId) || 0;
                workerProgress.set(msg.workerId, msg.spins);
                progressDone += msg.spins - oldDone;
                ...
                if (completedWorkers === numWorkers) {
                    ...
                    const avgFsPerBaseSpin = globalTotalFsPlayed / TOTAL_SPINS;
                    const retriggerRatePerBaseSpin = (globalRetriggers / TOTAL_SPINS) * 100;
                    ...
                    console.log(`📈 Echte RTP:            ${rtp.toFixed(4)}%`);
                    console.log(`Ø Gewinn pro Spin:       ${avgWinX.toFixed(6)}x`);
                    console.log("--------------------------------------");
                    ...
                    console.log(`Ø FS pro Base Spin:      ${avgFsPerBaseSpin.toFixed(6)}`);
                    console.log(`Retrigger:               ${globalRetriggers.toLocaleString("de-DE")}`);
                    console.log(`Retrigger/BaseSpin:      ${retriggerRatePerBaseSpin.toFixed(6)}%`);
                    console.log("--------------------------------------");
